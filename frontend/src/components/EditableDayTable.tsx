...
export function EditableDayTable({
  dashboard,
  onRefresh,
  selectedMonth,
}: Props) {
  const { shifts, definitions, team_config, stations, station_definitions } =
    dashboard;
  ...
  const stationDefMap = Object.fromEntries(
    station_definitions.map((d) => [d.code, d]),
  );
  // date -> station entry
  const stationByDate = Object.fromEntries(
    stations.filter((s) => s.assigned_date).map((s) => [s.assigned_date!, s]),
  );

  const stdHours = team_config?.standard_hours ?? 6;

  function shiftColor(code: string) {
    return shiftDefs[code]?.color || "var(--text-3)";
  }
  function stationLabel(code: string) {
    return stationDefMap[code]?.label || code;
  }
  ...
  function getEdit(s: Shift): Edit {
    return (
      edits[s.id] ?? {
        ...
        notes: s.notes ?? "",
        actual_time_start: s.actual_time_start ?? "",
        actual_time_end: s.actual_time_end ?? "",
      }
    );
  }
  ...
  function handleCodeChange(s: Shift, code: string) {
    ...
    const cur = (p: Record<number, Partial<Edit>>) => p[s.id] || {};
    setEdits((p) => ({
      ...p,
      [s.id]: {
        ...getEdit(s),
  ...
  function previewOt(
    s: Shift,
    e: Edit,
  ): { hours: number | null; ot: number | null } {
    ...
    return {
      hours: Math.round(hw * 10) / 10,
      ot: Math.round(Math.max(0, hw - stdHours) * 10) / 10,
    };
  }
  ...
  async function save(s: Shift) {
    ...
    try {
      ...
      showFlash(s.id, false, apiError(err));
    } finally {
      setSaving(null);
    }
  }
  ...
  const filtered = shifts.filter((s) => {
    ...
    return (
      s.shift_code.toLowerCase().includes(f) ||
      s.shift_label.toLowerCase().includes(f) ||
      s.shift_date.includes(f)
    );
  });
    ...
    >
      ...
      >
        <div>
          ...
          <h2 className="heading" style={{ fontSize: 18, marginTop: 4 }}>
            Modifica turni
          </h2>
          <div className="muted" style={{ fontSize: 11, marginTop: 1 }}>
            Ore ordinarie: {stdHours}h/giorno
          </div>
        </div>
        <input
          className="input"
      ...
      {filtered.length === 0 ? (
        <div className="empty-state">
          <div style={{ fontSize: 13 }}>Nessun turno</div>
        </div>
      ) : (
        <div
          style={{
                  ...
                  <tr key={shift.id}>
                      ...
                      {station && (
                        <div
                          style={{
                            fontSize: 9,
                            color:
                              stationDefMap[station.station_code]?.color ||
                              "var(--accent-2)",
                            marginTop: 1,
                          }}
                        >
                    ...
                    <td>
                      ...
                      >
                        ...
                        <select
                          ...
                          value={e.shift_code}
                          onChange={(ev) =>
                            handleCodeChange(shift, ev.target.value)
                          }
                        >
                          {definitions.map((d) => (
                          ...
                          {!shiftDefs[e.shift_code] && (
                            <option value={e.shift_code}>{e.shift_code}</option>
                          )}
                        </select>
                      </div>
                    </td>
                    <td>
                      <input
                        ...
                        value={e.shift_label}
                        onChange={(ev) =>
                          setField(shift.id, shift, {
                            shift_label: ev.target.value,
                          })
                        }
                      />
                    </td>
                    ...
                    <td>
                      <input
                        className="input"
                        type="time"
                        style={{ padding: "3px 5px", fontSize: 11, width: 82 }}
                        value={e.actual_time_start}
                        placeholder={shift.time_start?.slice(0, 5) || ""}
                        onChange={(ev) =>
                          setField(shift.id, shift, {
                            actual_time_start: ev.target.value,
                          })
                        }
                      />
                    </td>
                    <td>
                      <input
                        ...
                        onChange={(ev) =>
                          setField(shift.id, shift, {
                            actual_time_end: ev.target.value,
                          })
                        }
                      />
                    </td>
                    <td style={{ fontSize: 11, whiteSpace: "nowrap" }}>
                      {hours !== null ? (
                        <span>
                          ...
                          {ot !== null && ot > 0 && (
                            <span
                              style={{ color: "var(--warning)", marginLeft: 3 }}
                            >
                              +{ot}h
                            </span>
                          )}
                    ...
                    <td>
                      {isDirty ? (
                        ...
                        <span
                          style={{
                            fontSize: 10,
                            color: flash.ok ? "var(--accent)" : "var(--danger)",
                          }}
                        >
                          {flash.text}
                        </span>
