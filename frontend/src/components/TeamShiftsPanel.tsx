...
export function TeamShiftsPanel({ currentUser, definitions, stationDefs, standardHours, onRefresh }: Props) {
  ...
  async function load() {
    setLoading(true);
    try { const { data } = await api.get<TeamMemberShifts[]>("/manager/team/shifts"); setTeamShifts(data); }
  ...
  function numDaysInView() { return daysInMonth(filterYear, filterMonth); }

  function openModal(shift: Shift, userName: string) {
  ...
  function handleEditCode(code: string) {
    ...
    setEditShift({ ...editShift, shift_code: upper, shift_label: def ? def.label : editShift.shift_label });
  }
  ...
  function previewOt(): string {
    ...
    const ot = Math.max(0, hw - standardHours);
  ...
  async function saveEdit() {
    ...
    try {
      ...
      setTimeout(() => { setSelectedShift(null); load(); }, 900);
  ...
  async function saveManual() {
    ...
    if (errors === 0) { setShowManual(false); setManualAssignments({}); load(); if (onRefresh) onRefresh(); }
  }
          ...
          <div className="fade-in" style={{ padding: 14, borderRadius: "var(--radius-sm)", background: "rgba(110,231,183,.06)", border: "1px solid rgba(110,231,183,.2)", marginBottom: 16 }}>
            ...
            {manualUserId && (
              <div style={{ overflowX: "auto", marginBottom: 12 }}>
                <div style={{ display: "grid", gridTemplateColumns: `80px repeat(${daysInMonth(manualYear, manualMonth)}, 54px)`, gap: 4, minWidth: "max-content" }}>
                  ...
                  <div style={{ fontSize: 11, color: "var(--text-2)", alignSelf: "center", fontWeight: 500, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {members.find(m => String(m.id) === manualUserId)?.full_name?.split(" ")[0] || "Turno"}
                  </div>
                  {Array.from({ length: daysInMonth(manualYear, manualMonth) }, (_, i) => i+1).map(day => {
                    ...
                    return (
                      <select key={day}
                        style={{
                          width: 54, height: 40, borderRadius: 8, border: `2px solid ${code ? color : "var(--border)"}`,
                          background: code ? `${color}22` : "rgba(255,255,255,.04)",
                          color: code ? color : "var(--text-3)", fontWeight: 700, fontSize: 12, textAlign: "center",
                          cursor: "pointer", outline: "none", fontFamily: "var(--font-display)",
                        }}
                        value={code}
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button className="btn btn-ghost btn-sm" onClick={() => { setShowManual(false); setManualAssignments({}); setManualStatus(null); }}>Annulla</button>
            </div>
          </div>
        )}
            ...
            <table style={{ borderCollapse: "separate", borderSpacing: 0, minWidth: "max-content" }}>
              <thead>
                <tr>
                  ...
                  <th style={{
                    ...
                    padding: "6px 12px 6px 0", fontSize: 10, color: "var(--text-3)", fontWeight: 700,
                    textAlign: "left", borderRight: "1px solid var(--border)", whiteSpace: "nowrap",
                  }}>Dipendente</th>
                  {days.map(day => (
              ...
              <tbody>
                {teamShifts.map((member, memberIdx) => (
                  <tr key={member.user.id} style={{ marginTop: memberIdx > 0 ? 6 : 0 }}>
                    {/* Sticky name cell */}
                    <td style={{
                      position: "sticky", left: 0, zIndex: 1,
                      background: memberIdx % 2 === 0 ? "var(--bg-2)" : "var(--bg-1)",
                      padding: "8px 12px 8px 0", borderRight: "1px solid var(--border)",
                      paddingTop: memberIdx > 0 ? 14 : 8,
                    }}>
                      <div style={{ fontSize: 12, fontWeight: 600, whiteSpace: "nowrap", color: "var(--text)" }}>
                    ...
                    {days.map(day => {
                      ...
                      return (
                        <td key={day} style={{ padding: "2px", paddingTop: memberIdx > 0 ? 10 : 2, verticalAlign: "middle" }}>
                          <div
                            onClick={() => shift && openModal(shift, member.user.full_name)}
                            title={shift ? `${shift.shift_label} · ${shift.shift_date}${hasOt ? ` · +${shift.overtime_hours}h stra.` : ""}` : ""}
                            style={{
                              width: 36, height: 36, borderRadius: 7,
                              display: "flex", alignItems: "center", justifyContent: "center",
                              background: shift ? `${color}22` : "rgba(255,255,255,.03)",
                              border: `1px solid ${shift ? `${color}55` : "rgba(255,255,255,.06)"}`,
                              cursor: shift ? "pointer" : "default",
                              fontSize: 10, fontWeight: 800, color: shift ? color : "var(--text-3)",
        ...
        {definitions.map(d => (
          <div key={d.code} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11 }}>
            ...
            <span style={{ color: "var(--text-2)" }}>
              {d.label}
              {d.time_start && <span style={{ color: "var(--text-3)", marginLeft: 4 }}>{d.time_start.slice(0,5)}–{d.time_end?.slice(0,5)}</span>}
            </span>
          </div>
        ))}
      </div>
          ...
          <div className="modal" style={{ maxWidth: 480 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div>
                ...
                <div className="muted" style={{ fontSize: 12 }}>
                  {selectedShift.shift.time_start
                    ? `${selectedShift.shift.time_start.slice(0,5)} – ${selectedShift.shift.time_end?.slice(0,5)}`
                    : "Non impostato"}
                </div>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
